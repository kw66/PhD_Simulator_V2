...
开始页和游戏信息页显示“访客 总数（今日）”与“访问 总数（今日）”。沿用个人主页的 Supabase `increment_counter` / `get_counters` 接口，V2 使用独立的 `phd_simulator_v2_` 计数器及本地存储键，不与个人主页或 V1 混算。接口使用与个人主页相同的公开匿名密钥。

“访问”按页面打开或刷新计数，切换栏目、开局、重开和游戏内操作不重复计数。“访客”通过当前浏览器的本地标记去重，总访客首次访问计一次，今日访客每天计一次；当天按北京时间划分。清除浏览器数据、无痕模式、换浏览器或禁用存储会影响去重，统计不代表精确的真实人数。
